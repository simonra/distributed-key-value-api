...
| [ASPNETCORE_DETAILEDERRORS](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/host/web-host?view=aspnetcore-8.0#detailed-errors) | `True` | Enables detailed error responses, which are useful when debugging why your API fails, but is generally considered a bad idea to enable in production, because nefarious users could more easily get more info on how to break your setup. |
...
| KV_API_ENCRYPT_DATA_ON_KAFKA | <ul><li>`"true"`</li><li>not set</li></ul> | Whether the data that goes to/comes from Kafka is encrypted. Affects both producing and consuming. If set, you also have to specify the key in the `KV_API_AES_KEY` config variable or a path to a file containing the key in the `KV_API_AES_KEY_LOCATION` config variable |
| KV_API_AES_KEY | 64 hex characters (`0-9a-fA-F`, 256 bits) | If encrypting the data on the topic, you can set the key here. Only used if the `KV_API_ENCRYPT_DATA_ON_KAFKA` environment variable has been set to `"true"`. Overrides the content obtained from `KV_API_AES_KEY_LOCATION` if both are set. |
